...
void replay(CBResult myCB, behavior* head) 
{
	...
	if (head == NULL || myCB.CBList == NULL) { printf("error!\n"); return; }
	...
	if (order == 1)
	{
		...
		while (temp_bh->next != NULL)
		{
			...
			if (temp.CBList != NULL) temp_ct->myCB = CB_copy(temp); else temp_ct->myCB = ERRORCB;
			temp_ct->bef = bef_ct;
			temp_ct->next = NULL;
		...
		while (1) {
			...
			else if (i == 75 && temp_bh->bef != NULL)
			{
				...
				temp_ct = temp_ct->bef;
				if (temp_ct != NULL) chessboard_print(temp_ct->myCB); else { printf("error!\n"); return; }
			}
			else if (i == '\r') 
			{
				system("cls");
...
CBResult get_order(CBResult myCB)
{
	...
	while ((i = getch()) != 0) {
		...
		if (i == '0') {
			...
			if (temp.column != 0) {
				myCB = temp;
				myBH->drawOrMark = 0; myBH->x = x; myBH->y = y; myBH->next = NULL; myBH->bef = bef; bef->next = myBH; bef = myBH;
			}
			break;
		}
		if (i == '1') {
			...
			if (temp.column != 0) {
				myCB = temp;
				myBH->drawOrMark = 1; myBH->x = x; myBH->y = y; myBH->next = NULL; myBH->bef = bef; bef->next = myBH; bef = myBH;
			}
			break;
		}
	}
